...
export default function AdminSidebarIntegrationStatus({ title, statusUrl }: Props) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        setPayload({
          ok: Boolean(data.ok),
          label: data.label ?? (data.ok ? "Connected" : "Not configured"),
          detail: data.detail,
  ...
  return (
    <div className="admin-sidebar-db-status" role="status" aria-live="polite">
      <div className="admin-sidebar-db-status-row">
        ...
        <span className="admin-sidebar-db-status-title">{title}</span>
      </div>
