...
export default function AdminSidebarTikTokStatus() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const res = await fetch("/api/admin/tiktok-status", { cache: "no-store" });
        ...
        setPayload({
          ok: Boolean(data.ok),
          label: data.label ?? (data.ok ? "Active" : "Not configured"),
          detail: data.detail,
  ...
  return (
    <div className="admin-sidebar-db-status" role="status" aria-live="polite">
      <div className="admin-sidebar-db-status-row">
        ...
        <span className="admin-sidebar-db-status-title">TikTok Pixel</span>
      </div>
      <div className="admin-sidebar-db-status-line">
        {state === "loading" ? "Checking…" : (payload?.label ?? "—")}
      </div>
      {payload?.secondaryLabel && state !== "loading" ? (
        <div className="admin-sidebar-db-status-meta">{payload.secondaryLabel}</div>
      ) : null}
