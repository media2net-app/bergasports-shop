...
export default function HomeHeroBanner({ blocks }: Props) {
  ...
  return (
    <section className="relative isolate w-full overflow-hidden bg-[#1a1a1a]">
      ...
      />
      <div
        className="absolute inset-0 bg-gradient-to-r from-black/85 via-black/55 to-black/25 md:via-black/45 md:to-transparent"
        aria-hidden
      />
      ...
      <div className="relative mx-auto flex min-h-[min(72vh,640px)] w-full max-w-[1440px] flex-col justify-end px-4 py-14 sm:min-h-[min(68vh,720px)] sm:justify-center sm:py-20 md:px-8 lg:px-10">
        <div className="max-w-xl">
          <p className="text-xs font-bold uppercase tracking-[0.22em] text-[var(--brand-mid)] md:text-sm">
            {hero.eyebrow ?? SITE_SLOGAN}
          </p>
          <h1 className="mt-3 whitespace-pre-line font-[family-name:var(--font-heading)] text-3xl font-semibold leading-[1.1] tracking-tight text-white sm:text-4xl md:text-5xl lg:text-[3.25rem]">
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
            <Link
              href="/shop"
              className="inline-flex items-center justify-center rounded-full bg-[var(--brand-mid)] px-7 py-3 text-sm font-bold text-[#1a1a1a] transition hover:bg-[#f2d680]"
            >
